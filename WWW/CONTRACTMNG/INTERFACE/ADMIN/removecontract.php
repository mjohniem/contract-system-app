...
-->
		...
		<p style="background-color:#11685E; text-align:center; font-size:24px; font:Georgia, 'Times New Roman', Times, serif">Please fill the form below to remove the tenders to the web page</p>
      ...
      <div id="sub" style=" text-align:center"><input name="submit" type="submit"  id="submit" tabindex="5" value="REMOVE THE CONTRACT" style="background-color:#7CE9CB; border:double solid; text-align:center">
...
try{
		...
		$count=$con->prepare("SELECT  rname FROM contract WHERE rname=:rname");
		$count->bindParam(":rname",$refname);
		$count->execute();
		$no=$count->rowCount();
		if($no>0 )
			{		 
				$sql="DELETE FROM contract WHERE rname=:rname";
